...
def main():
    ...
    with st.spinner('Loading Data...'):
        ...
        train_num = st.sidebar.slider('Training by how many data', min_value=60, max_value=len(df_confirm), value=200,
                                      step=10)
        pred_num = st.sidebar.slider('Predicting how many days', min_value=30, max_value=60, value=30, )
        ...
        if country_name == 'Global':
            ...
            c1.info(locale.format_string("%d", df_confirm.iloc[-1, -1], grouping=True))
            c1.dataframe(
            ...
            c2.info(locale.format_string("%d", df_death.iloc[-1, -1], grouping=True))
            c2.dataframe(
            ...
            st.altair_chart(draw_trend(df_death,show_global,model, num_observations))
            ...
            u1.info(locale.format_string("%d", df_confirm[country_name][-1], grouping=True))
            u1.dataframe(
                pd.DataFrame(shift_diff(df_confirm)[country_name][-7:]).rename(columns={country_name: 'Last_7_Days'}))
            u2.write('Total Death Cases:')
            u2.info(locale.format_string("%d", df_death[country_name][-1], grouping=True))
            u2.dataframe(
...
def select_top_country(df, top_num=10, days=7):
    ...
    df = pd.DataFrame((df.iloc[-1, :-1] - df.iloc[-1 * days, :-1]).sort_values(ascending=False).head(top_num),
                      columns=[col_name])
    df[col_name] = df[col_name].apply(lambda x: locale.format_string("%d", x, grouping=True))
    return df
...
def draw_trend(df, show_global,model, num_obv=50):
    ...
    if show_global:
        ...
        localize = lambda x: locale.format_string("%d", x, grouping=True)
...
class arima():
    ...
    def _create_df_plot(self, col_type ='Cases'):
        ...
        line = pd.DataFrame(self.model_fit.forecast(self.pred_num)[0],
                            #index=self.model_fit.predict(1, self.pred_num + self.train_num).index[-self.pred_num:],
    ...
    def draw_single_trend(self, return_chart=False,country_name = 'Cases'):
        # df_plot = confirm_plot.append(death_plot, ignore_index=True)
